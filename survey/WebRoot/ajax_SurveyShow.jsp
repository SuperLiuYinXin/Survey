...
<%
	...
	Survey survey = sdao.findSurvey(Integer.valueOf(request
			.getParameter("sid")));
	QuestionDAO qdao = DAOFactory.getQuestionDAO();
	List<Question> qlist = qdao.listAllQuestion(Long.valueOf(request
			.getParameter("sid")));
	//String obj=request.getParameter("obj");
	String to = request.getParameter("to");
 System.out.println(qlist.size());
	qlist: for (Question q : qlist) {
		...
		case 1:
			...
			System.out.println(q.getQJdtz().toString());
			sz_jdtz = q.getQJdtz().split("&");
			//Jdtz=0;	
			for (int i = 0; i < body.length; i++) {
				if ("over".equals(sz_jdtz[i]))
					out.println("<li><input type='radio' name='answer"
							+ q.getQId() + "' value='" + i
							+ "' onclick='EndSurvey(" + q.getQId()
							+ ")'>" + body[i] + "</li>");
				else if ("null".equals(sz_jdtz[i])) {
					out.println("<li><input type='radio' name='answer"
							+ q.getQId() + "' value='" + i
							+ "' onclick=showNext('qbox" + q.getQId()
							+ "'," + (q.getQId() + 1) + ")>" + body[i]
							+ "</li>");
					//	Jdtz++;
				} else {
		...
		case 2:
			for (int i = 0; i < body.length; i++) {
				out.println("<li><input type='checkbox' name='answer"
						+ q.getQId() + "' value='" + i + "'>" + body[i]
						+ "</li>");
			}
			out.println("</ul><br/>");
			//out.println("<div id=qbox"+q.getQId()+"></div>");
		...
		case 3:
			...
			for (int i = 0; i < body.length; i++) {
				if ("over".equals(sz_jdtz[i]))
					out.print("<li><input type='radio' name='answer"
							+ q.getQId() + "' value='" + i
							+ "' onclick='EndSurvey(" + q.getQId()
							+ ")'>" + body[i] + "</li>");
				else if ("null".equals(sz_jdtz[i])) {
		...
		case 4:
			...
			out.println("<input type='text' name='txtanswer"
					+ q.getQId() + "'>");
			out.println("</ul><br/>");
			//out.println("<div id=qbox"+q.getQId()+"></div>");
			break;
		case 5:
			...
			out
					.println("<textarea name='answer"
							+ q.getQId()
							+ "' rows=4 cols=40 onpropertychange='this.style.posHeight=this.scrollHeight'></textarea>");
			out.println("</ul><br/>");
			//out.println("<div id=qbox"+q.getQId()+"></div>");
			break;
		}
