...
public class SurveyManage extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if ("AddSurvey".equals(request.getParameter("op"))) {
			...
			survey.setTemplet(0);// Ԥ��ģ�幦��
			if (request.getParameter("Survey_ipRepeat") == "true") {
				survey.setSIpRepeat('1');
			} else {
				survey.setSIpRepeat('0');
			}
			if (request.getParameter("Survey_isOpen") == "true") {
				survey.setSIsOpen('1');
			} else {
				survey.setSIsOpen('0');
			}
			...
			survey.setSHits(0);
			survey.setSIsAudited('0');
			survey.setSUsehits(0);
			boolean ret = surveydao.addSurvey(survey);
			// System.out.println(ret);
			if (ret == true)
				response
						.sendRedirect("../admin/OpResult.jsp?op=SurveyAdd&ret=true");
			else
				response
						.sendRedirect("../admin/OpResult.jsp?op=SurveyAdd&ret=false");

		} else if ("SurveyAudi".equals(request.getParameter("op"))) {
			Boolean audit = Boolean.valueOf(request.getParameter("audit"));
			SurveyDAO surveydao = DAOFactory.getSurveyDAO();
			synchronized (mutex1) {
				Survey survey = surveydao.findSurvey(Integer.valueOf(request
						.getParameter("sid")));
				if (audit == true)
					survey.setSIsAudited('1');
				else
					survey.setSIsAudited('0');
				...
				if (ret == true)
					...
					response
							.sendRedirect("../admin/OpResult.jsp?op=SurveyAudi&ret=false");
			}
			...
			synchronized (mutex2) {
				...
				if (request.getParameter("Survey_isPassword") != null) {
					survey.setSPassword(request
							.getParameter("Survey_isPassword"));
				} else
				...
				// System.out.println(ret);
				if (ret == true)
					response.sendRedirect("../admin/SurveyEdit.jsp?sid=" + sid
							+ "&words=" + URLEncoder.encode("操作成功！", "UTF-8"));
				else
					response
							.sendRedirect("../admin/OpResult.jsp?op=SurveyEdit&ret=false");

			}
		} else if ("DelSurvey".equals(request.getParameter("op"))) {
			int surveyId = Integer.valueOf(request.getParameter("sid"));
			SurveyDAO surveydao = DAOFactory.getSurveyDAO();
			char ret1 = surveydao.delSurvey(surveyId);
			...
			if (ret1 == '1')
				...
				response
						.sendRedirect("../admin/OpResult.jsp?op=SurveyDel&ret=false");
		}
