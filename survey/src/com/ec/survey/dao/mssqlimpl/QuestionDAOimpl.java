...
public class QuestionDAOimpl implements QuestionDAO {
	...
	public Question findQuestion(Long questionId) {
		...
		try {
			if (rs.next()) {
				...
				question.setSurvey(rs.getInt("s_id"));
				...
				question.setQOrder(rs.getInt("q_order"));
	...
	public char updateQuestion(Question question) {
		Connection conn = ConnectionFactory.getConnection();
		PreparedStatement pstmt = null;
		String sql = "UPDATE question SET s_id=?, "
				+ "q_type=?, q_head=?, q_body=?, q_result=?, "
				+ "q_img=?, q_jdtz=?, q_order=?" + " WHERE q_id=?";
		try {
			pstmt = conn.prepareStatement(sql);
	...
	public char delQuestions(int surveyId) {
		SQLCommand cmd = new SQLCommand();

		if (-1 != cmd.executeSQL("delete from question where s_id=" + surveyId)) {
			return '1';
		} else {
			return '0';
		}
	...
	public List<Question> listQuestions(String WhereClause) {
		SQLCommand cmd = new SQLCommand();
		RowSet rs = cmd.queryRowSet("select * from question where "
				+ WhereClause);
		Question question;
		List<Question> list = new ArrayList<Question>();
		try {
