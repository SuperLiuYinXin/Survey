package com.ec.survey.dao.mssqlimpl;

import java.sql.Connection;
import java.sql.PreparedStatement;
import java.sql.SQLException;
import java.util.ArrayList;
import java.util.List;

import javax.servlet.http.HttpServletRequest;
import javax.sql.RowSet;

import com.ec.survey.dao.AnswersheetDAO;
import com.ec.survey.dto.Answersheet;
import com.ec.survey.dto.Question;
import com.ec.survey.dto.Survey;
import com.swufe.pager.PageConfig;
import com.swufe.sql.ConnectionFactory;
import com.swufe.sql.SQLCommand;

public class AnswersheetDAOimpl implements AnswersheetDAO {
	private List list_answersheet = null;

	public char addAnswersheet(Answersheet answersheet) {
		Connection conn = ConnectionFactory.getConnection();
		PreparedStatement pstmt = null;
		String sql = "INSERT INTO answersheet(s_id, as_result,"
				+ "as_postdate,as_userIP) VALUES(?, ?, ?,?)";
		try {
			pstmt = conn.prepareStatement(sql);
			pstmt.setLong(1, answersheet.getSurvey());
			pstmt.setString(2, answersheet.getAsResult());
			pstmt.setDate(3, new java.sql.Date(answersheet.getAsPostdate()
					.getTime()));
			pstmt.setString(4, answersheet.getAsUserIp());

			return pstmt.executeUpdate() == 1 ? '1' : '0';

		} catch (SQLException e) {

			e.printStackTrace();
			return '0';
		} finally {
			SQLCommand.close(pstmt);
			SQLCommand.close(conn);
		}
	}

	public char delAnswersheet(Long answersheetId) {
		SQLCommand cmd = new SQLCommand();
		 if(-1 != cmd.executeSQL("delete from answersheet where as_id="
				+ answersheetId))
		{
			return '1';
		}else{
			return '0';
		}
	}

	public Answersheet findAnswersheet(Long answersheetId) {
		SQLCommand cmd = new SQLCommand();
		RowSet rs = cmd.queryRowSet("select * from answersheet where as_id="
				+ answersheetId);

		Answersheet answersheet = new Answersheet();
		try {
			if (rs.next()) {
				answersheet.setAsId(rs.getLong("as_id"));
				answersheet.setSurvey(rs.getLong("s_id"));
				answersheet.setAsResult(rs.getString("as_result"));
				answersheet.setAsPostdate(rs.getDate("as_postdate"));
				answersheet.setAsUserIp(rs.getString("as_userip"));
			}
			return answersheet;

		} catch (SQLException e) {

			e.printStackTrace();
			return null;
		}

	}

	public List<Answersheet> listAllAnswersheet(Long surveyId) {
		SQLCommand cmd = new SQLCommand();
		RowSet rs = cmd.queryRowSet("select * from answersheet where s_id="
				+ surveyId + " order by as_postdate desc");
		Answersheet answersheet;
		List<Answersheet> list = new ArrayList<Answersheet>();
		try {
			while (rs.next()) {
				answersheet = new Answersheet();
				answersheet.setAsId(rs.getLong("as_id"));
				answersheet.setSurvey(rs.getLong("s_id"));
				answersheet.setAsResult(rs.getString("as_result"));
				answersheet.setAsPostdate(rs.getDate("as_postdate"));
				answersheet.setAsUserIp(rs.getString("as_userip"));
				list.add(answersheet);
			}

			return list;

		} catch (SQLException e) {

			e.printStackTrace();
		}
		return null;
	}

	public List<?> doSelect(int recordStart, int sizePage, PageConfig pageConfig) {
		List<Answersheet> newlist = new ArrayList<Answersheet>();
		HttpServletRequest request = pageConfig.getRequest();
		List<Answersheet> list_answersheet = this.listAllAnswersheet(Long
				.valueOf(request.getParameter("sid")));
		for (int i = recordStart; i < recordStart + sizePage; i++) {
			if (i < list_answersheet.size())
				newlist.add(list_answersheet.get(i));
			else
				break;

		}
		return newlist;
	}

	public int getCount(PageConfig pageConfig) {
		if (this.list_answersheet == null) {
			HttpServletRequest request = pageConfig.getRequest();
			list_answersheet = this.listAllAnswersheet(Long.valueOf(request
					.getParameter("sid")));
		}

		return list_answersheet.size();
	}

	public char delAnswersheets(int sid) {
		SQLCommand cmd = new SQLCommand();
		if (-1 != cmd.executeSQL("delete from answersheet where s_id=" + sid)) {
			return '1';
		} else {
			return '0';
		}
	}

	public char isIpRepeate(int sid, String IP) {
		SQLCommand cmd = new SQLCommand();
		RowSet rs = cmd.queryRowSet("select s_id from answersheet where s_id="
				+ sid + " and as_userip='" + IP + "'");
		try {
			if (rs.next()) {
				return '1';
			} else
				return '0';
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			return '0';
		}
	}

}
